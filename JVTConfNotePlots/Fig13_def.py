...
if __name__ == "__main__":
    ...
    # ----------------- SIGNAL ----------------------------------------------------
    signalSelection = "fabs(J0ZDPhi) <1.2 && ZPt>30 && Jpt>20 && Jpt<50 && fabs(Jeta)<2.4 && Jindex==0"; doSignal = False
    # corrJVF leading Jet -----------------------------

    low = -1; high = 1.1
    plotter.GetHistoFromTree("JVFData",      tfileData,  tree , "JnPUTrkCorrJVF", "Weight*( 1                            &&"+signalSelection+")", 35, low, high)
    plotter.GetHistoFromTree("JVFMC_HS",     tfileMC  ,  tree , "JnPUTrkCorrJVF", "Weight*(   JisHS&&Jtruthpt>10         &&"+signalSelection+")", 35, low, high)
    plotter.GetHistoFromTree("JVFMC_nHS",    tfileMC  ,  tree , "JnPUTrkCorrJVF", "Weight*( !(JisHS&&Jtruthpt>10)        &&"+signalSelection+")", 35, low, high)
    plotter.GetHistoFromTree("JVFMC_PU",     tfileMC  ,  tree , "JnPUTrkCorrJVF", "Weight*( JisPU                        &&"+signalSelection+")", 35, low, high)
    plotter.GetHistoFromTree("JVFMC_other",  tfileMC  ,  tree , "JnPUTrkCorrJVF", "Weight*( !JisPU&&!(JisHS&&Jtruthpt>10)&&"+signalSelection+")", 35, low, high)

    plotter.MLfit("JVFMC_HS", "JVFMC_nHS", "JVFData", [-1,1.1], SigScaleFact)

    signal_scale_factor = plotter.fit_['sigScaleFact']
    bkg_scale_factor    = plotter.fit_['bkgScaleFact']

    plotter.SetMCScale(signal_scale_factor, ["JVFMC_HS"])
    plotter.SetMCScale(bkg_scale_factor,    ["JVFMC_PU","JVFMC_other"])

    plotter.DataToMC(["JVFMC_HS", "JVFMC_PU","JVFMC_other"], "JVFData" , "corrJVF", "Events", [lumiLabel,MClabel,jetlabel,"20<p_{T}<50 GeV, |#eta|<2.4", "Z p_{T}>30 GeV", "PU selection"], ["HS", "PU", "other"])
    ...
    plotter.DataToMC(["JVFMC_HS", "JVFMC_PU","JVFMC_other"], "JVFData" , "JVT", "Events", [lumiLabel,MClabel,jetlabel,"20<p_{T}<50 GeV, |#eta|<2.4", "Z p_{T}>30 GeV", "PU selection"], [ "HS", "PU", "other"])
    ...
    plotter.MLfit("JVFMC_HS", "JVFMC_nHS", "JVFData", [0,1.5], SigScaleFact)
    ...
    plotter.DataToMC(["JVFMC_HS", "JVFMC_PU","JVFMC_other"], "JVFData" , "R_{pT}", "Events", [lumiLabel,MClabel,jetlabel,"20<p_{T}<50 GeV, |#eta|<2.4", "Z p_{T}>30 GeV", "PU selection"], ["HS", "PU", "other"])
