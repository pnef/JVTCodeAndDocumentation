...
if __name__ == "__main__":
    ...
    selection             = ""
    treevariable          = [  "JJVF"  , "JnPUTrkCorrJVF"  ]  
    VarName               = [  "JVF"   , "corrJVF"         ] 
    nBins                 = [   500    ,  1000             ] 
    binMin                = [  -1      , -1                ] 
    binMax                = [  1.1     , 1.1               ] 
...
#    funkylabel         = "#DeltaR(#mu,jet)>1"
    ...
    if True:
        ...
        if ApplyWeights :
            treeSelectionNPVdep_PU = "Weight*(1 &&"+treeSelectionNPVdep_PU+")"
            treeSelectionNPVdep_HS = "Weight*(1 &&"+treeSelectionNPVdep_HS+")"
            print treeSelectionNPVdep_PU
            print treeSelectionNPVdep_HS
        ...
        for VarIndex,Var in zip(range(len(treevariable)),treevariable):
            print ">>>plotter: Getting 3D Histos for doEffFakeVsNPV" 
            plotter.GetHistoFromTree("PU_Var_EffFake_FromTree"+str(VarIndex), tfiles[VarIndex],  putree , Var+":Jpt:"+EffFakeVsVar, treeSelectionNPVdep_PU , 45,5,50,50,0,100,nBins[VarIndex],binMin[VarIndex],binMax[VarIndex])
            plotter.GetHistoFromTree("HS_Var_EffFake_FromTree"+str(VarIndex), tfiles[VarIndex],  hstree , Var+":Jpt:"+EffFakeVsVar, treeSelectionNPVdep_HS , 45,5,50,50,0,100,nBins[VarIndex],binMin[VarIndex],binMax[VarIndex])
        ...
        plotter.Draw(H_HS_Var_EffFake, 
                "N_{Vtx}", "Efficiency", False, True, [MClabel, jetlabel, etalabel+ptLegend, efflabel], 
                Cuts_Var_EffFake)
