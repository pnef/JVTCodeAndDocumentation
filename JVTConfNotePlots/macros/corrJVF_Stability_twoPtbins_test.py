...
if __name__ == "__main__":
    ...
    selection             = ""
    treevariable          = [  "JJVF"                  ,"JJVF"                    , "JnPUTrkCorrJVF"              , "JnPUTrkCorrJVF"]  
    VarName               = [  "JVF"                   ,"JVF"                     , "corrJVF"                     , "corrJVF"                        ] 
    nBins                 = [   500                    , 500                      ,  1000                         ,  1000           ] 
    binMin                = [  -1                      ,-1                        , -1                            , -1              ] 
    binMax                = [  1.1                     ,1.1                       , 1.1                           , 1.1             ] 
    HSonTheRight          = [  True                    ,True                      , True                          , True            ]
    tfiles                = [  fT                      ,fT                        , fT                            , fT              ]
    additionalCutPU       = [  "&&Jpt<30 && Jpt>20"    ,"&&Jpt<30 && Jpt>20"      , "&&Jpt<30 && Jpt>20"          , "&&Jpt<30 && Jpt>20"              ]
    additionalCutHS       = [  "&&Jpt<30 && Jpt>20"    ,"&&Jpt<30 && Jpt>20"      , "&&Jpt<30 && Jpt>20"          , "&&Jpt<30 && Jpt>20"              ]
    ...
    if True:
        ...
        for VarIndex,Var in zip(range(len(treevariable)),treevariable):
            ...
            if(HSonTheRight[VarIndex]):
                plotter.EfficiencyVsNPV("HS_Var_EffFake_FromTree"+str(VarIndex), 2, -99, -99, cut,  -99, "HS_Var_EffFake_effVal"+str(VarIndex))
                plotter.EfficiencyVsNPV("PU_Var_EffFake_FromTree"+str(VarIndex), 4, -99, -99, cut,  -99, "PU_Var_EffFake_effVal"+str(VarIndex))
            else:
                plotter.EfficiencyVsNPV("HS_Var_EffFake_FromTree"+str(VarIndex), 2, -99, -99, -99, cut,  "HS_Var_EffFake_effVal"+str(VarIndex))
                plotter.EfficiencyVsNPV("PU_Var_EffFake_FromTree"+str(VarIndex), 4, -99, -99, -99, cut,  "PU_Var_EffFake_effVal"+str(VarIndex))
        ...
        plotter.Draw(H_HS_Var_EffFake, 
                "N_{Vtx}", "Efficiency", False, True, [MClabel, jetlabel, etalabel, efflabel, "solid markers: 20 < p_{T} < 30 GeV","open markers: 30 < p_{T} < 40 GeV" ], 
                Cuts_Var_EffFake)
