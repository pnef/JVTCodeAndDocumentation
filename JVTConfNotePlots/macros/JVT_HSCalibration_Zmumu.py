...
def PUJetDefFlatness():
    ...
    plotter.GetHistoFromTree("JVFMC_PU1",     tfileMC  ,  tree ,"fabs(J0ZDPhi)" , "Weight*(JdRTruthPt4>0.4                          &&"+JetSelection+"&&ZPt>30 && ZPt<40)", 10, 0, 3.15)
    plotter.GetHistoFromTree("JVFMC_PU2",     tfileMC  ,  tree ,"fabs(J0ZDPhi)" , "Weight*(JisPU                                    &&"+JetSelection+"&&ZPt>30 && ZPt<40)", 10, 0, 3.15)
...
#    plotter.H_["JVFMC_PU2"].h_.Fit("myf_PU2","R","",0,2)
    plotter.Draw(["JVFMC_PU1","JVFMC_PU2"], "|#Delta#phi(Z,jet)|", "Entries", False, True, [lumiLabel,MClabel,jetlabel,"20<p_{T}<50 GeV, |#eta|<2.4", "40 < Z p_{T} < 50 GeV"],["PU (#DeltaR>0.4)", "PU (#DeltaR>0.6)"])
...
if __name__ == "__main__":
    ...
    JetSelection = "ZPt>20 && Jpt>20 && Jpt<50 && fabs(Jeta)<2.4 && Jindex==0 " 
    Variable     = "ZPt"
    VarName      = "p_{T}^{ref} [GeV]"
    VariableNBins= 8
    VariableLow  = 20
    VariableHigh = 60
    ...
    for ibin in range(1,plotter.H_["dPhi_nHS"].h_.GetNbinsX()+1):
        binlow  = plotter.H_["dPhi_nHS"].h_.GetBinLowEdge(ibin)
        binhigh = plotter.H_["dPhi_nHS"].h_.GetBinLowEdge(ibin)+plotter.H_["dPhi_nHS"].h_.GetBinWidth(ibin)
        ...
        eta [ibin-1]         = plotter.H_["dPhi_nHS"].h_.GetBinCenter(ibin) 
    ...
    plotter.Draw(["PUcontamination", "PUcontaminationTruth"], VarName, "N_{j}^{PU} in signal region", False, True, [lumiLabel, MClabel, jetlabel, ptlabel], ["MC prediction", "MC"])
    ...
    for ibin in range(1,plotter.H_["dPhi_data"].h_.GetNbinsX()+1):
        binlow  = plotter.H_["dPhi_data"].h_.GetBinLowEdge(ibin)
        binhigh = plotter.H_["dPhi_data"].h_.GetBinLowEdge(ibin)+plotter.H_["dPhi_data"].h_.GetBinWidth(ibin)
        plotter.SetRange("dPhi_data", "x", binlow, binhigh, "htmp")
        plotter.SetRange("dPhi_HS",  "x", binlow, binhigh, "htmpHS")
        plotter.Projector("htmp",   "y", "htmp")
        plotter.Projector("htmpHS", "y", "htmpHS")
        bindPhiLimitBkg = plotter.H_["htmp"].h_.FindBin(PUControl_dPhiMax)
        D_eta [ibin-1]         = plotter.H_["dPhi_data"].h_.GetBinCenter(ibin) 
        ...
        print "data", plotter.H_["dPhi_data"].h_.GetBinCenter(ibin) , 
    ...
    plotter.Draw(["PUcontamination", "PUcontaminationTruth","PUcontaminationData"], 
                 VarName, "N_{j}^{PU} in signal region", False, True, [lumiLabel, MClabel, jetlabel, ptlabel], ["MC prediction", "MC","Data"])
    ...
    for JVTcut in CUTVal:
        ...
        plotter.Draw(["EffTruth", "EffTruth_2"], VarName, "Efficiency", False, True, [lumiLabel, "Z#rightarrow#mu#mu", jetlabel, ptlabel, "JVT > "+str(JVTcut)], ["MC, Sherpa", "MC, PowHeg"])
        
        plotter.Clone("EffTruth","EffTruth_diff")
        plotter.H_["EffTruth_diff"].h_.Add(plotter.H_["EffTruth_2"].h_, -1)
        plotter.Divide("EffTruth_diff","EffTruth", "EffTruth_diff_frac")
        plotter.Draw(["EffTruth_diff"],      VarName, "Efficiency", False, True, [lumiLabel, "Z#rightarrow#mu#mu", jetlabel, ptlabel, "JVT > "+str(JVTcut)], ["Sherpa - Powheg"])
        plotter.Draw(["EffTruth_diff_frac"], VarName, "#Delta #epsilon / #epsilon", False, True, [lumiLabel, "Z#rightarrow#mu#mu", jetlabel, ptlabel, "JVT > "+str(JVTcut)], ["(Sherpa - Powheg) / Sherpa" ])
        plotter.Clone("EffTruth_diff_frac","EffTruth_diff_frac_smooth")
        plotter.H_["EffTruth_diff_frac_smooth"].h_.Smooth(20)
        plotter.Draw(["EffTruth_diff_frac","EffTruth_diff_frac_smooth"], VarName, "#Delta #epsilon / #epsilon", False, True, [lumiLabel, "Z#rightarrow#mu#mu", jetlabel, ptlabel, "JVT > "+str(JVTcut)], ["nominal", "smoothed" ])
        ...
        plotter.Draw(["EffTruth","EffReco","EffRecoCorr"],VarName, "HS efficiency", False, True, [lumiLabel, MClabel, jetlabel, ptlabel], ["MC", "MC reco uncorr", "MC reco corr"])
        ...
        plotter.Draw(["EffTruth","EffReco","EffRecoCorr", "EffData"], VarName , "HS efficiency", False, True, [lumiLabel, MClabel, jetlabel, ptlabel], ["MC truth", "MC reco uncorr", "MC reco corr", "data uncorr"])
        ...
        plotter.Draw(["EffTruth","EffReco","EffRecoCorr", "EffData","EffDataCorr"], VarName , "HS efficiency", False, True, 
                     [lumiLabel, MClabel, jetlabel, ptlabel], ["MC truth", "MC reco uncorr", "MC reco corr", "data uncorr", "data"])

        # final efficiency plot 
        plotter.Draw(["EffTruth", "EffTruth_2", "EffRecoCorr", "EffDataCorr"], VarName , "Efficiency", False, True, 
                     [lumiLabel, MClabel, jetlabel, ptlabel, "JVT > "+str(JVTcut)], ["MC", "MC Powheg", "MC pred",  "Data"])
        
        # final efficiency plot 
        plotter.SetAtlasInfo("Simulation Internal")
        plotter.Draw(["EffTruth","EffRecoCorr"],  VarName , "Efficiency", False, True, 
                     [lumiLabel, MClabel, jetlabel, ptlabel,"JVT > "+str(JVTcut)], ["MC", "MC pred"  ])
        
        # final efficiency plot 
        plotter.SetAtlasInfo("Preliminary")
        plotter.Draw(["EffRecoCorr", "EffDataCorr"], VarName , "Efficiency", False, True, 
                     [lumiLabel, MClabel, jetlabel, ptlabel, "JVT > "+str(JVTcut)], ["MC",  "Data"])
        
        plotter.drawMCsum_=False
        plotter.SetAtlasInfo("Preliminary")
        plotter.DataToMC(["EffRecoCorr"], "EffDataCorr" , VarName, "Efficiency", [lumiLabel, MClabel, jetlabel, ptlabel, "JVT > "+str(JVTcut)], ["MC"])
        ...
        for ibin in range(1, plotter.H_["EffRecoCorrSyst"].h_.GetNbinsX()+1):
            ...
            systsmooth  = math.fabs(plotter.H_["EffTruth_diff_frac_smooth"].h_.GetBinContent(ibin))* plotter.H_["EffRecoCorrSyst"].h_.GetBinContent(ibin)
        ...
        plotter.DataToMCSyst("EffRecoCorrSyst","EffRecoCorr","EffDataCorr" ,       VarName, "Efficiency", [lumiLabel, MClabel, jetlabel, ptlabel,"JVT > "+str(JVTcut)], ["Total Uncertainty old", "MC", "Data"])
        plotter.DataToMCSyst("EffRecoCorrSystSmooth","EffRecoCorr","EffDataCorr" , VarName, "Efficiency", [lumiLabel, MClabel, jetlabel, ptlabel,"JVT > "+str(JVTcut)], ["Total Uncertainty", "MC", "Data"])
        ...
        plotter.DataToMCSyst("EffRecoCorrSyst","EffRecoCorr","EffDataCorr",  VarName, "Efficiency", [lumiLabel, MClabel, jetlabel, ptlabel,"JVT > "+str(JVTcut)], ["Total Uncertainty", "MC", "Data", "MC HS"], ["EffVeryTruth"])
