...
if __name__ == "__main__":
    ...
    Zsel     = "40 < Z p_{T} < 50 GeV"

    # parameters ---------------------------
    ScaleFact    = 20.3*1.2*1000*1.2 # additional 20% scale for Sherpa
    JetSelection = "ZPt>20 && Jpt>20 && Jpt<50 && fabs(Jeta)<2.4 && Jindex==0 &&ZPt>40 && ZPt<50" 
    Variable     = "Jeta"
    VarName      = "jet #eta"
    VariableNBins= 10
    VariableLow  = -2.4
    ...
    for JVTcut in CUTVal:
        ...
        for ibin in range(1, plotter.H_["EffRecoCorrSyst"].h_.GetNbinsX()+1):
            ...
            print "bin", ibin, "eff", plotter.H_["EffRecoCorrSyst"].h_.GetBinContent(ibin), "stat", stat, "syst", syst, "tot", total, "totalFrac", totalFrac
            plotter.H_["EffRecoCorrSyst"].h_.SetBinError(ibin, total)

        
        plotter.DataToMCSyst("EffRecoCorrSyst","EffRecoCorr","EffDataCorr" , VarName, "Efficiency", [lumiLabel, MClabel, jetlabel, ptlabel,Zsel,"JVT > "+str(JVTcut)], ["Total Uncertainty", "MC", "Data"])
