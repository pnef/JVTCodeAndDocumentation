...
if __name__ == "__main__":
    ...
    putree                = "JetTree0"
    hstree                = "JetTree0"
    treeSelectionNPVdep_PU= " NPV>=5 && JisPU &&  fabs(Jeta)<2.4 && Jpt<30 && Jpt>20 && VtxDzTruth<0.1 "
    treeSelectionNPVdep_HS= " NPV>=5 && JisHS &&  fabs(Jeta)<2.4 && Jpt<30 && Jpt>20 && VtxDzTruth<0.1 && Jtruthpt >10"
    selection             = ""
    treevariable          = [   "kNN100trim_pt20to50_Likelihood", "kNN100trim_pt20to50_Likelihood", "kNN100trim_pt20to50_Likelihood"]  
    VarName               = [   "JVT>0.2"                       , "JVT>0.4"                       , "JVT>0.7"                       ] 
    nBins                 = [    1000                           ,  1000                           ,  1000                           ] 
    binMin                = [   -0.1                            , -0.1                            , -0.1                            ] 
    binMax                = [   1.1                             , 1.1                             , 1.1                             ] 
    CutValues             = [   0.2                             , 0.4                             , 0.7                             ]
    ...
    if True:
        ...
        for VarIndex,Var in zip(range(len(treevariable)),treevariable):
            ...
            if(HSonTheRight[VarIndex]):
                plotter.EfficiencyVsNPV("HS_Var_EffFake_FromTree"+str(VarIndex), 2, -99, -99, CutValues[VarIndex],  -99, "HS_Var_EffFake_effVal"+str(VarIndex))
                plotter.EfficiencyVsNPV("PU_Var_EffFake_FromTree"+str(VarIndex), 4, -99, -99, CutValues[VarIndex],  -99, "PU_Var_EffFake_effVal"+str(VarIndex))
            else:
                plotter.EfficiencyVsNPV("HS_Var_EffFake_FromTree"+str(VarIndex), 2, -99, -99, -99, CutValues[VarIndex],  "HS_Var_EffFake_effVal"+str(VarIndex))
                plotter.EfficiencyVsNPV("PU_Var_EffFake_FromTree"+str(VarIndex), 4, -99, -99, -99, CutValues[VarIndex],  "PU_Var_EffFake_effVal"+str(VarIndex))

            # format 
            plotter.SetRange("HS_Var_EffFake_effVal"+str(VarIndex), "x", 5, 32,"HS_Var_EffFake_effVal"+str(VarIndex))
            plotter.SetRange("PU_Var_EffFake_effVal"+str(VarIndex), "x", 5, 32,"PU_Var_EffFake_effVal"+str(VarIndex))
        ...
        plotter.Draw(H_HS_Var_EffFake, 
                "N_{Vtx}", "Efficiency", False, True, [MClabel, jetlabel, "20 < p_{T} < 30 GeV,"+etalabel ], 
                Cuts_Var_EffFake)
        plotter.Draw(H_PU_Var_EffFake, 
                "N_{Vtx}", "Fake Rate",  False, True, [MClabel, jetlabel, "20 < p_{T} < 30 GeV,"+etalabel], 
                Cuts_Var_EffFake)
