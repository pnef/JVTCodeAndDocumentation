...
class Plotter:
    def __init__(self, MultiProcess=False):
        ...
        ROOT.gROOT.ProcessLine(".L MT2Helpers.hh");
        ROOT.gROOT.ProcessLine(".x SetStyle_MT2Ratio.C");
        ROOT.gROOT.ProcessLine(".x SetStyle_MT2.C");
    ...
    def Set2DCanvas(self, YesNo, specialPalette=False):
        if YesNo:
            if specialPalette:
                ROOT.gROOT.ProcessLine(".x SetStyle_MT2_2D_newPalette.C");
                ROOT.gROOT.ForceStyle()
            else:
                ROOT.gROOT.ProcessLine(".x SetStyle_MT2_2D.C");
                ROOT.gROOT.ForceStyle()
        else:
            ROOT.gROOT.ProcessLine(".L MT2Helpers.hh");
            ROOT.gROOT.ProcessLine(".x SetStyle_MT2Ratio.C");
...
#        print [resultMC, resultData]
    ...
    def Draw(self, Hlist, xtitle="x", ytitle="y", normalized=False, ATLASlogo=False, labels=[], legends=False):
        ...
        leg = ROOT.TLegend(0.6, 0.85-legnentries*0.04, 0.8, 0.85);
