...
if __name__ == "__main__":
    ...
    putree                = "JetTree0"
    hstree                = "JetTree0"
    treeSelection_PU      = " NPV>=5 && JisPU &&  fabs(Jeta)<2.4 && Jpt<50 && Jpt>30 && VtxDzTruth<0.1  "
    treeSelection_HS      = " NPV>=5 && JisHS &&  fabs(Jeta)<2.4 && Jpt<50 && Jpt>30 && VtxDzTruth<0.1 && Jtruthpt >10  "
    treeSelectionROC_PU   = " NPV>=5 && JisPU &&  fabs(Jeta)<2.4 && Jpt<50 && Jpt>30 && VtxDzTruth<0.1 "
    treeSelectionROC_HS   = " NPV>=5 && JisHS &&  fabs(Jeta)<2.4 && Jpt<50 && Jpt>30 && VtxDzTruth<0.1 && Jtruthpt >10"
    treeSelectionNPVdep_PU= " NPV>=5 && JisPU &&  fabs(Jeta)<2.4 && Jpt<50 && Jpt>30 && VtxDzTruth<0.1 "
    treeSelectionNPVdep_HS= " NPV>=5 && JisHS &&  fabs(Jeta)<2.4 && Jpt<50 && Jpt>30 && VtxDzTruth<0.1 && Jtruthpt >10"
    selection             = ""
    treevariable          = [  "JstdJVF",   "JnPUTrkCorrJVF" ,  "JHSPVtrkSumOverPt"  , "kNN100trim_pt20to50_Likelihood" ]  
    VarName               = [  "JVF"    ,   "corrJVF"        ,  "R_{pT}"             , "JVT"                        ] 
    nBins                 = [   300     ,    300             ,  300                  , 300                          ] 
    binMin                = [  -1       ,   -1               ,  -1                   , -0.2                         ] 
    binMax                = [  1.1      ,   1.1              ,  1.1                  , 1.2                          ] 
...
#    funkylabel         = "#DeltaR(#mu,jet)>1"
    ...
    if True:
        ...
        for VarIndex,Var in zip(range(len(treevariable)),treevariable):
            ...
            plotter.H_["Var_ROC_fromTree_"+VarName[VarIndex]].h_.GetXaxis().SetRangeUser(0.85,0.99)
