...
if __name__ == "__main__":
    ...
    fT   =path+"20140305.08.12_PileUpStudies_DiLeptonSelection_351830.Zmumu_MC12_Sherpa_COMMON.jetmet2012pileupcustom.root"
    
    # Draw Variable ------------------------------------------------------------------------
    putree                = "JetTree0"
    hstree                = "JetTree0"
    treeSelection_PU      = " NPV>=5 && JisPU &&  fabs(Jeta)<2.4 && Jpt>20 && Jpt<30    "
    treeSelection_HS      = " NPV>=5 && JisHS &&  fabs(Jeta)<2.4 && Jpt>20 && Jpt<30   && Jtruthpt >10  "
    treeSelectionROC_PU   = " NPV>=5 && JisPU &&  fabs(Jeta)<2.4 && Jpt>20 && Jpt<30   "
    treeSelectionROC_HS   = " NPV>=5 && JisHS &&  fabs(Jeta)<2.4 && Jpt>20 && Jpt<30   && Jtruthpt >10"
    treeSelectionNPVdep_PU= " NPV>=5 && JisPU &&  fabs(Jeta)<2.4 && Jpt>20 && Jpt<30   "
    treeSelectionNPVdep_HS= " NPV>=5 && JisHS &&  fabs(Jeta)<2.4 && Jpt>20 && Jpt<30   && Jtruthpt >10"
    selection             = ""
...
#    funkylabel         = "#DeltaR(#mu,jet)>1"
    ...
    if True:
        ...
        for VarIndex,Var in zip(range(len(treevariable)),treevariable):
            ...
            plotter.GetHistoFromTree("PU_Var_ROC_FromTree_"+VarName[VarIndex], tfiles[VarIndex],  putree , Var+":Jpt:NPV", SelectionROC_PU , NbinsNPV, NPVmin, NPVmax ,100,0,500,nBins[VarIndex],binMin[VarIndex],binMax[VarIndex])
            plotter.GetHistoFromTree("HS_Var_ROC_FromTree_"+VarName[VarIndex], tfiles[VarIndex],  hstree , Var+":Jpt:NPV", SelectionROC_HS , NbinsNPV, NPVmin, NPVmax ,100,0,500,nBins[VarIndex],binMin[VarIndex],binMax[VarIndex])
            ...
            if VarIndex==0:
                plotter.H_["Var_ROC_fromTree_"+VarName[VarIndex]].setdrawstyle("AP")
            else:
...
#            plotter.H_["Var_ROC_fromTree_"+VarName[VarIndex]].h_.GetXaxis().SetRangeUser(0., 1.1)
        ...
        marker.SetMarkerColor(ROOT.kAzure+10)
        marker.SetX(0.9)
        marker.SetY(0.1)
        marker.Draw("same")
        canvas.Update()
