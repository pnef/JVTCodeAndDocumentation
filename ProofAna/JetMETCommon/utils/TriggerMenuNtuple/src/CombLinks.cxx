...
CombLinks::CombLinks() : mRoIType(-1), mLastStep(-1) {
}
...
const FeatureIndex* CombLinks::index(const std::string& feature) const {
   //cout << "CombLinks: find feature " << feature << endl;
  int id = featureId(feature);
  //cout << "CombLinks: found feature index " << index(id) << endl;
  return index(id);
}
...
const FeatureIndex* CombLinks::index(int feature_id) const {
  ...
  if (p != mIndexMap.end()) {
    return &(p->second);
     ...
     cout << "No feature " << featureName(feature_id) << " found in RoIType " << mRoIType << endl;
  }
  return 0;
}

const vector<FeatureIndex>* CombLinks::indexVec(const string& feature) const {
   //cout << "CombLinks: find featureVec " << feature << endl;
  int id = featureId(feature);
  //cout << "CombLinks: found featureVec index " << indexVec(id) << endl;
  return indexVec(id);
}
