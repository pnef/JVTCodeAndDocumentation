...
void TMNUtil::initForD3PD(TTree* menu_tree, TTree* t_config) {
   if(pelle) cout << "initForD3PD" << endl;
...
void TMNUtil::readChainFeatureMapping(TTree* t, int ientry) {
   ...
   if (t) {
      ...
      for (int ii=0; ii<c2->GetEntries(); ++ii) {
         ...
         if (s) {
            if(pelle) cout << "Feature: " << s->GetString().Data() << endl;
...
void TMNUtil::readTrigConfMetadata(TTree* t_config) {
   map<std::string, int>* l1map=0;
   map<std::string, int>* hltmap=0;
   if(pelle) cout << "readTrigConfMetadata" << endl;

   if (t_config) {
      t_config->SetBranchAddress("LVL1NameMap", &l1map);
      t_config->SetBranchAddress("HLTNameMap", &hltmap);
      t_config->GetEntry(0);
      if (hltmap) {
         if(pelle) cout << "readTrigConfMetadata: in hltmap" << endl;
         int id=0;
         map<std::string, int>::const_iterator p;
         for (p=hltmap->begin(); p!=hltmap->end(); ++p) {
            id = p->second;
            if (p->first.find("EF_") == 0) id += ChainEntry::kEFChainIdOffset;
            ChainEntry::addChain(p->first, id);
            if(pelle) cout << "readTrigConfMetadata: in hltmap chain " << p->first << " id " << id << endl;

// 	std::cout << "readTrigConfMetaData::Chain: " << p->first 
// 		  << ", id: " << id << std::endl; 
         }
      }
   } else {
      cout << "D3PD TrigConf meta data TTree is null" << endl;
   }
}
...
void TMNUtil::reconstructChainEntryRoILinks(std::vector<ChainEntry>& chains, 
   ...
   if(pelle) cout << " reconstructChainEntryRoILinks called" << endl;
   ...
   if(pelle) cout << " reconstructChainEntryRoILinks: found " << nChains << " nChains" << endl;
   ...
   for (i=0; i<nChains; ++i) {
      ...
      if(pelle) cout << " reconstructChainEntryRoILinks:Adding chain: " << ChainEntry::chainName(id) << " for id " << id  << " with " << (*mBranchData_vvi[mStrChain_RoIType])[i].size() << " RoIs" << endl;
      for (j=0; j<(*mBranchData_vvi[mStrChain_RoIType])[i].size(); ++j) {
         ...
         if(pelle) cout << "reconstructChainEntryRoILinks: addRoI " << j << " at index " << (*mBranchData_vvi[mStrChain_RoIIndex])[i][j] << " and type " << (*mBranchData_vvi[mStrChain_RoIType])[i][j] << endl;
      }
   }
...
// 	       << " status_vec=" << status_br << endl;
 ...
 //	cout << "DEBUG:: create pointer for branch" << (*pf) << endl;
...
// 	       << " status_vec=" << status_br << endl;
         ...
         if(pelle) cout << "tmp links: " << tmp_links << endl;
         roiLinks.addCombLinks(tmp_links);
      }
      if(pelle)cout << "Reconstructed " << roiLinks.getCombLinks().size() << " RoIs" << endl;
...
const std::vector<int> TMNUtil::getIndex(std::string feature, std::string ChainName, bool ActiveOnly){
   ...
   for(pChain=mChainEntry.begin(); pChain!=mChainEntry.end(); ++pChain){
      if( pChain->getName()==ChainName ) {
         if(pelle) cout << "getIndex: found chain " << ChainName << endl;
         std::vector<CombLinks> Combs = mRoILinks.getCombLinks(*pChain);
         if(pelle) cout << "getIndex: found " << Combs.size() << " CombLinks/RoIs" << endl;
         std::vector<CombLinks>::const_iterator pComb;
         for(pComb=Combs.begin(); pComb!=Combs.end(); ++pComb) {
            const FeatureIndex* fIndex = pComb->index(feature);
            if(pelle) cout << "getIndex: fIndex at " << fIndex << endl;
